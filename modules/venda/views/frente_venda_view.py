...
class FrenteVendaView(QWidget, Ui_FrenteVenda):
    ...
    def _configurar_venda_inicial(self) -> None:
        ...
        if self._cliente_padrao_venda() == "CONSUMIDOR_FINAL":
            ...
            self._cliente_selecionado_manualmente = False
            self.lblClienteNome.setText("Selecionar cliente")
    ...
    def _aplicar_consumidor_final(self) -> None:
        self._cliente_atual = dict(self._cliente_consumidor_final) if self._cliente_consumidor_final else None
        self._cliente_selecionado_manualmente = True
        self.lblClienteNome.setText(str((self._cliente_atual or {}).get("nome") or "Consumidor Final"))

    def _cliente_eh_consumidor_final(self) -> bool:
        cliente_id = int((self._cliente_atual or {}).get("id") or 0)
        consumidor_final_id = int((self._cliente_consumidor_final or {}).get("id") or 0)
        return consumidor_final_id > 0 and cliente_id == consumidor_final_id
        ...
        self.pagamento_solicitado.emit(
            {
                ...
                "cliente_nome": self.lblClienteNome.text().strip() or "Consumidor Final",
                "cliente_eh_consumidor_final": self._cliente_eh_consumidor_final(),
    ...
    def _abrir_desconto(self) -> None:
        if self._regra_desconto_venda() == REGRA_DESCONTO_EXIGIR_AUTORIZACAO:
        ...
        if modo == "item":
            ...
            if self._prioridade_promocional() == PRIORIDADE_DESCONTO_ANTES_PROMOCAO:
    ...
    def _cliente_padrao_venda(self) -> str:
        return str(
            self._parametros_venda.get("cliente_padrao_venda") or CLIENTE_PADRAO_CONSUMIDOR_FINAL
        ).strip().upper()

    def _regra_desconto_venda(self) -> str:
        return str(
            self._parametros_venda.get("regra_desconto_venda") or REGRA_DESCONTO_PERMITIR
        ).strip().upper()
    ...
    def _prioridade_promocional(self) -> str:
        return str(
            ...
            or PRIORIDADE_PROMOCAO_ANTES_DESCONTO
        ).strip().upper()
