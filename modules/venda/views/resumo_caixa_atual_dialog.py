    ...
    def _popular_pagamentos(self) -> None:
        ...
        if totais:
            ...
            for row_index, forma in enumerate(totais):
                nome = QTableWidgetItem(str(forma.get("forma_pagamento") or "Forma"))
                qtd = QTableWidgetItem(f"{int(forma.get('qtd_vendas') or 0)} venda(s)")
                valor = QTableWidgetItem(formatar_moeda(float(forma.get("total") or 0.0)))
                valor.setTextAlignment(int(Qt.AlignRight | Qt.AlignVCenter))
                self.tablePagamentos.setItem(row_index, 0, nome)
                self.tablePagamentos.setItem(row_index, 1, qtd)
                self.tablePagamentos.setItem(row_index, 2, valor)
        else:
